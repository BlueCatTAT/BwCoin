...
class PayethController extends Controller_Abstract{
    ...
    public $DyxConiType = "ETH_LBTH_0x042f972ac93404f0fcbe4e3a0729f0b395232106";
...
EOD;
    ...
    public function withdrawlog(){
        ...
        $data['appKey'] = $this->appkey;
        $data['coinType'] = "ETH_LBTH_0x042f972ac93404f0fcbe4e3a0729f0b395232106";
        $data['page'] = 1;
        $result = $this->createLinkstringUrlencode($data);
        $data['sign'] = $this->sha256($result, $this->appsecrect);
        $result2 = json_encode($data,320);
...
//';
        ...
        if($result['direction'] == 'in' &&  $result['type'] == "cold"){
            ...
            $userinfo = $this->db->field("*")->table("y_user")->where(" lbth_address = '{$address}'")->find();
            $rechdata['lbth_num'] = sprintf("%.3f",$result['qty']);
            $rechdata['create_time'] = substr($result['updatetime'],0,-3);
            $rechdata['address'] = $address;
            $rechdata['status'] = 1;
            if(strpos($result['coinType'], 'LBTH') !== false){
                $rechdata['currency'] = "LBTH";
                if(!empty($userinfo)){
                    $rechdata['u_id'] = $userinfo['id'];
                    $rechuser['lbth_num'] = bcadd($userinfo['lbth_num'], $result['qty'], 3);
                    $rechuser['lbth_available_balance'] = bcadd($userinfo['lbth_available_balance'], $result['qty'], 3);
                    ...
                    $log['type'] = "充值";
                    $log['info'] = "成功{$rechdata['lbth_num']}个DYX";
					$log['num'] = $rechdata['lbth_num'];
                    $log['create_time'] = substr($result['updatetime'],0,-3);
                    $log['currency_name'] = "DYX";
                ...
                if(!empty($userinfo)){
                    ...
                    $log['user_id'] = $userinfo['id'];
                    $log['type'] = "充值";
                    $log['info'] = "成功{$rechdata['lbth_num']}个USDT";
					$log['num'] = $rechdata['lbth_num'];
                    $log['create_time'] = substr($result['updatetime'],0,-3);
                    $log['currency_name'] = "USDT";
                    $this->db->action($this->db->insertSql("user_num_log",$log));
                }else{
                    //没有用户操作
                    $rechdata['u_id'] = 0;
                ...
                if(!empty($userinfo)){
                    ...
                    $log['user_id'] = $userinfo['id'];
                    $log['type'] = "充值";
                    $log['info'] = "成功{$rechdata['lbth_num']}个{$result['coinType']}";
					$log['num'] = $rechdata['lbth_num'];
                    $log['create_time'] = substr($result['updatetime'],0,-3);
                    $log['currency_name'] = $result['coinType'];
                    $this->db->action($this->db->insertSql("user_num_log",$log));
                }else{
                    //没有用户操作
                    $rechdata['u_id'] = 0;
            ...
            if(!empty($orderinfo)){
                ...
                $price = $orderinfo['lbth_num'];
                $type = $result['coinType'];
                if(strpos($type, 'LBTH') !== false){
                    $userdata['lbth_num'] = $userinfo['lbth_num'] - $price;
                    $userdata['lbth_freeze_balance'] = $userinfo['lbth_freeze_balance'] - $price;
                    $this->db->action($this->db->updateSql('user',$userdata,"id = {$id}"));
                    $log['user_id'] = $id;
                    $log['type'] = "提现";
                    $log['info'] = "成功{$price}个LBTH";
					$log['num'] = $price;
                    $log['create_time'] = substr($result['updatetime'],0,-3);
                    $log['currency_name'] = "LBTH";
                    $this->db->action($this->db->insertSql("user_num_log",$log));
