...
class dbModel{
    ...
    public function joinlink($join_link = "INNER",$table=null,$join=null){
        $exp = "/[\da-zA-Z]+_/";
        preg_match($exp,$table,$data);
        if(!empty($data)){
            $this->sqlstr .= $join_link." JOIN ".$table." ON {$join} ";
        }else{
            $this->sqlstr .= $join_link." JOIN ".$this->dbconfig["prefix"].$table." ON {$join} ";
        }
        return $this;
    }
    //SQL链式操作的分组查询
    ...
    //求长度
    public function count($sql,$type = 2){
        $data = $this->action($sql,$type);
        if($data){
            return count($data);
        }else{
            return 0;
    ...
    //求单个对应key值 (1维数组)
    public function getByKey($table,$key,$where = null){
        if($where == null){
            $sql = "SELECT {$key} FROM ".$this->dbconfig["prefix"].$table;
        }else{
            $sql = "SELECT {$key} FROM ".$this->dbconfig["prefix"].$table." WHERE {$where} ";
        }
        $result = $this->yafpdo->query($sql);
        $data = $result->fetch(2);
        if($data){
    ...
    //求单个对应key值 (2维数组)
    public function getByKeys($table,$key,$where = null){
        if($where == null){
            $sql = "SELECT {$key} FROM ".$this->dbconfig["prefix"].$table;
        }else{
            $sql = "SELECT {$key} FROM ".$this->dbconfig["prefix"].$table." WHERE {$where} ";
